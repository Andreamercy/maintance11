...
struct ServiceTaskListView: View {
    ...
    private var emptyState: some View {
        VStack(spacing: 16) {
            ...
            Text("No service tasks").font(.subheadline).foregroundStyle(.appTextSecondary)
            Spacer()
    ...
    var body: some View {
        ...
        return VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack {
                    Text("Checklist").font(.caption.weight(.medium)).foregroundStyle(.appTextSecondary)
                    Spacer()
                    Text("\(checked)/\(total)").font(.caption.weight(.bold)).foregroundStyle(.appTextPrimary)
                }
